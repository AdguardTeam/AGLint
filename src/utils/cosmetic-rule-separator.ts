...
export class CosmeticRuleSeparatorUtils {
    ...
    public static isAdblockPlusSnippet(separator: string): boolean {
        return (
            separator == CosmeticRuleSeparator.AbpSnippet ||
            separator == CosmeticRuleSeparator.AbpSnippetException
        );
    }
    ...
    public static isUblockScriptlet(separator: string): boolean {
        return (
            separator == CosmeticRuleSeparator.uBoScriptlet ||
            separator == CosmeticRuleSeparator.uBoScriptletException
        );
    }
    ...
    public static isAdGuardScriptlet(separator: string): boolean {
        return (
            separator == CosmeticRuleSeparator.AdgScriptlet ||
            separator == CosmeticRuleSeparator.AdgScriptletException
        );
    }
    ...
    public static isAdGuardJs(separator: string): boolean {
        return (
            separator == CosmeticRuleSeparator.AdgJsInject ||
            separator == CosmeticRuleSeparator.AdgJsInjectException
        );
    }
    ...
    public static isUblockHtml(separator: string): boolean {
        return (
            separator == CosmeticRuleSeparator.uBoHTML ||
            separator == CosmeticRuleSeparator.uBoHTMLException
        );
    }
    ...
    public static isAdGuardHtml(separator: string): boolean {
        return (
            separator == CosmeticRuleSeparator.AdgHTML ||
            separator == CosmeticRuleSeparator.AdgHTMLException
        );
    }
    ...
    /**
     * Looks for the cosmetic rule separator in the rule. This is a simplified version that masks the recursive function.
     *
     * @param {string} rule - Raw rule
     *
     * @returns {CosmeticRuleSeparatorResult} data
     * @returns {number} data[0] - Separator start position.
     * @returns {number} data[1] - Separator end position.
     * @returns {string | null} data[2] - The separator itself.
     * @returns {boolean | null} data[3] - Is exception separator.
     */
    ...
    /**
     * Recursively looks for the cosmetic rule separator in the rule. The basic idea is from TSUrlFilter, this is a further optimized version.
     * The method is very low level, but it is necessary for speed.
     *
     * @param {string} rule - Raw rule.
     ...
     * @param {boolean} noHashMark - Once we have established that there is no hashmark character (#), it is unnecessary to search for it again in the next step of the recursion.
     * @param {boolean} noDollarSign - once we have established that there is no dollar sign character ($), it is unnecessary to search for it again in the next step of the recursion.
     *
     * @returns {CosmeticRuleSeparatorResult} data
     * @returns {number} data[0] - Separator start position.
     * @returns {number} data[1] - Separator end position.
