...
/**
 * Check if the error is an instance of SyntaxMatchError.
 *
 * @param possibleError Error to check.
 *
 * @returns `true` if the error is an instance of SyntaxMatchError.
 */
export const isCssTreeSyntaxMatchError = (possibleError: unknown): possibleError is SyntaxMatchError => {
    if (!possibleError || !(possibleError instanceof Error)) {
        return false;
    }
    ...
    return possibleError.name === 'SyntaxMatchError';
};
