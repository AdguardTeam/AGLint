...
describe('CssInjectionBodyParser', () => {
    ...
    test('parseAdgCssInjection', () => {
        ...
        // Invalid cases
        // eslint-disable-next-line max-len
        expect(() => CssInjectionBodyParser.parseAdgCssInjection('body > section[ad-source] { remove: true; remove: true; }')).toThrowError(/^Multiple remove property found in the following CSS injection body:/);

        // eslint-disable-next-line max-len
        expect(() => CssInjectionBodyParser.parseAdgCssInjection('body > section[ad-source] { remove: true; padding: 0; }')).toThrowError(
            /^In addition to the remove property, the following CSS injection body also uses other properties:/,
        );

        // eslint-disable-next-line max-len
        expect(() => CssInjectionBodyParser.parseAdgCssInjection('body > section[ad-source] { padding: 0; remove: true; }')).toThrowError(
            /^In addition to the remove property, the following CSS injection body also uses other properties:/,
        );

        expect(() => CssInjectionBodyParser.parseAdgCssInjection(
    ...
    test('generate', () => {
        ...
        expect(() => CssInjectionBodyParser.generate(
            {
                selectors: [<SelectorPlain>CssTree.parsePlain('.test', CssTreeParserContext.selector)],
            },
            AdblockSyntax.Abp,
